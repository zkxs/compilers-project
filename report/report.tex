...
\usepackage{longtable} % used by pandoc for tables
\usepackage{tabulary} % used by pandoc for tables
\usepackage{booktabs} % used by pandoc
\usepackage[utf8]{inputenc} % allows mapping of unicode characters to LaTeX
...
\newcommand\processmarkdownfile[1]
    {\immediate\write18{bash md2tex.sh #1}%
     \input{tmp_#1.tex}}

% Unicode mappings
\DeclareUnicodeCharacter{2192}{$\rightarrow$}
\DeclareUnicodeCharacter{3F5}{$\epsilon$}
\DeclareUnicodeCharacter{21B4}{$\searrow$}
...
Before work on the parser could even begin, the grammar had to undergo significant massaging.  

Grammar with our project-specific modifications applied and all productions split

\processmarkdownfile{grammar_3_split.md}

Grammar after epsilon removal

\processmarkdownfile{grammar_4_remove_epsilon.md}

Grammar after removal of left recursion

\processmarkdownfile{grammar_5_remove_left_recursion.md}

Grammar after left-factoring, final clean-up, and computation of firsts and follows

\processmarkdownfile{grammar_8_final.md}
